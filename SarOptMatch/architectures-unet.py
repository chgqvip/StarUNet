...
class DualAxis(tf.keras.layers.Layer):
    def __init__(self, input_size=64, channels=128, d_h=32, d_v=32, d_w=32, heads=8, dropout=0):
        ...
        self.d_w = d_w
        self.heads = heads
        self.dwconv_qh = DWCONV(channels)
        self.dwconv_kh = DWCONV(channels)
        self.pool_qh = tf.keras.layers.AveragePooling2D(pool_size=(1, input_size), strides=(1, 1), padding='valid')
        self.pool_kh = tf.keras.layers.AveragePooling2D(pool_size=(1, input_size), strides=(1, 1), padding='valid')
        self.fc_qh = tf.keras.layers.Dense(heads * d_h)
        self.fc_kh = tf.keras.layers.Dense(heads * d_h)

        self.dwconv_v = DWCONV(channels)
        self.fc_v = tf.keras.layers.Dense(heads * d_v)

        self.dwconv_qw = DWCONV(channels)
        self.dwconv_kw = DWCONV(channels)
        self.pool_qw = tf.keras.layers.AveragePooling2D(pool_size=(input_size, 1), strides=(1, 1), padding='valid')
        self.pool_kw = tf.keras.layers.AveragePooling2D(pool_size=(input_size, 1), strides=(1, 1), padding='valid')
        self.fc_qw = tf.keras.layers.Dense(heads * d_w)
        self.fc_kw = tf.keras.layers.Dense(heads * d_w)

        self.fc_o = tf.keras.layers.Dense(channels)

        self.dropout = tf.keras.layers.Dropout(dropout)
        self.scaled_factor_h = d_h ** -0.5
        self.scaled_factor_w = d_w ** -0.5
        self.Bh = self.add_weight(shape=(1, heads, input_size, input_size), trainable=True)
        self.Bw = self.add_weight(shape=(1, heads, input_size, input_size), trainable=True)
    ...
    def call(self, x):
        ...
        # Horizontal queries and keys
        qh = self.dwconv_qh(x)  # [b, h, w, c]
        qh = self.pool_qh(qh)  # [b, h, 1, c]
        qh = tf.squeeze(qh, axis=2)  # [b, h, c]
        qh = self.fc_qh(qh)  # [b, h, heads * d_w]
        qh = tf.reshape(qh, (b, self.heads, h, self.d_w))  # [b, heads, h, d_w]

        kh = self.dwconv_kh(x)  # [b, h, w, c]
        kh = self.pool_kh(kh)  # [b, h, 1, c]
        kh = tf.squeeze(kh, axis=2)  # [b, h, c]
        kh = self.fc_kh(kh)  # [b, h, heads * d_w]
        kh = tf.reshape(kh, (b, self.heads, h, self.d_w))  # [b, heads, h, d_w]

        # Vertical queries and keys
        qw = self.dwconv_qw(x)  # [b, h, w, c]
        qw = self.pool_qw(qw)  # [b, 1, w, c]
        qw = tf.squeeze(qw, axis=1)  # [b, w, c]
        qw = self.fc_qw(qw)  # [b, h, heads * d_w]
        qw = tf.reshape(qw, (b, self.heads, w, self.d_h))  # [b, heads, w, d_h]

        kw = self.dwconv_kw(x)  # [b, h, w, c]
        kw = self.pool_kw(kw)  # [b, 1, w, c]
        kw = tf.squeeze(kw, axis=1)  # [b, w, c]
        kw = self.fc_kw(kw)  # [b, h, heads * d_w]
        kw = tf.reshape(kw, (b, self.heads, w, self.d_h))  # [b, heads, w, d_h]

        # Compute horizontal attention
        attn_h = tf.einsum('bhjd,bhkd->bhjk', qh, kh)
        attn_h = attn_h * self.scaled_factor_h + self.Bh
        attn_h = tf.nn.softmax(attn_h, axis=-1)

        # Compute vertical attention
        attn_w = tf.einsum('bwjd,bwkd->bwjk', qw, kw)
        attn_w = attn_w * self.scaled_factor_w + self.Bw
        attn_w = tf.nn.softmax(attn_w, axis=-1)

        # Values
        v = self.dwconv_v(x)  # [b, h, w, c]
        v = tf.reshape(v, (b, h * w, c))  # [b, h*w, c]
        v = self.fc_v(v)  # [b, h*w, heads*self.d_v]
        v = tf.reshape(v, (b, self.heads, h, w * self.d_v))  # [b, heads, h, w*d_v]

        # Apply horizontal attention to values
        result_h = tf.einsum('bhjk,bhkw->bhjw', attn_h, v)

        # Apply vertical attention
        result_v = tf.einsum('bhjk,bhkd->bhjd', attn_w, result_h)

        # Project back to original dimensionality
        result = self.fc_o(tf.reshape(result_v, (b, h * w, self.heads * self.d_v)))
        result = tf.reshape(result, (b, h, w, c))

        return result


class FFN_MultiLN(tf.keras.layers.Layer):
    def __init__(self, in_channels, img_size, R, drop=0.):
        super(FFN_MultiLN, self).__init__()
        exp_channels = in_channels * R
        self.h = img_size
        self.w = img_size
        self.fc1 = tf.keras.layers.Dense(exp_channels)
        self.dwconv = DWCONV(exp_channels)
        self.ln1 = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        self.ln2 = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        self.ln3 = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        self.act1 = tf.keras.layers.Activation('relu')
        self.fc2 = tf.keras.layers.Dense(in_channels)

    def call(self, x):
        x = self.fc1(x)
        b, n, c = x.shape  # assuming x.shape is [batch, height*width, channels]

        h = x  # Save for residual connections

        # Reshape to [batch, height, width, channels], already in 'channels_last' format
        x = tf.reshape(x, (b, self.h, self.w, c))
        x = self.dwconv(x)
        x = tf.reshape(x, (b, self.h * self.w, c))  # Flatten back to [batch, height*width, channels]

        # Add and normalize, LayerNorm is channel-wise
        x = self.ln1(x + h)
        x = self.ln2(x + h)
        x = self.ln3(x + h)
        x = self.act1(x)
...
class IntraTransBlock(tf.keras.Model):
    def __init__(self, img_size, stride, d_h, d_v, d_w, num_heads, R=4, in_channels=46):
        super(IntraTransBlock, self).__init__()
        # Lightweight MHSA
        self.SlayerNorm = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        self.ElayerNorm = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        self.lmhsa = DualAxis(img_size, in_channels, d_h, d_v, d_w, num_heads, 0.0)
        # Inverted Residual FFN
        self.irffn = FFN_MultiLN(in_channels, img_size, R)

    def call(self, x):
        x_pre = x  # Tensor shape: (B, H, W, C)
        # Use tf.shape to handle dynamic shapes properly
        shape = tf.shape(x)
        b, h, w, c = shape[0], shape[1], shape[2], shape[3]

        # Apply first layer norm and MHSA
        x = tf.reshape(x, (b, h * w, c))
        x = self.SlayerNorm(x)
        x = tf.reshape(x, (b, h, w, c))
        x = self.lmhsa(x)
        x = x_pre + x  # Residual connection

        x_pre = x

        # Apply second layer norm and inverted residual FFN
        x = tf.reshape(x, (b, h * w, c))
        x = self.ElayerNorm(x)
        x = self.irffn(x)
        x = tf.reshape(x, (b, h, w, c))
...
class ScaleFormer(tf.keras.Model):
    def __init__(self):
        ...
        # self.decoder1 = DecoderBlock(512, 128)
        # self.decoder2 = DecoderBlock(256, 64)
        # self.decoder3 = DecoderBlock(128, 32)
    ...
    def call(self, inputs, return_features=False):
        ...
        x1_up = self.decoder1(encoder_skips[-2], encoder_skips[-3])
        x2_up = self.decoder2(x1_up, encoder_skips[-4])
        x3_up = self.decoder3(x2_up, encoder_skips[-5])

        x_final = self.decoder_final(x3_up, encoder_skips[-6])
