...
def _parse_files_grayscale(opt, sar, offsets):
    ...
    opt_decoded = tf.image.decode_png(opt_string, channels=3)  # 读取为RGB格式
    opt_rescaled = tf.image.convert_image_dtype(opt_decoded, dtype=tf.float32)

    # 判断光学图像是否已经是灰度图，如果不是则转换
    if opt_rescaled.shape[-1] == 3:  # 如果是RGB（三个通道）
        opt_grayscale = tf.image.rgb_to_grayscale(opt_rescaled)
    else:
        opt_grayscale = opt_rescaled  # 如果已经是灰度图，则直接使用

    # 读取并解码SAR图像
    sar_string = tf.io.read_file(sar)
    sar_decoded = tf.image.decode_png(sar_string, channels=1)  # 读取为灰度图
    ...
    # 创建高斯模糊的掩膜
    mask = gaussian_blur(tf.constant(4, dtype=tf.float32), offsets)

    return (opt_grayscale, sar_cropped), (mask, offsets) # 将 offsets 也作为目标的一部分返回
...
def _parse_files_RGB(opt, sar, offsets):
    ...
    return (opt_grayscale, sar_cropped), (mask, offsets)
